...
//        wind_gust_end_ = common::Time(wind_gust_start + wind_gust_duration);
        ...
        wind_sub_ = node_handle_->subscribe("/wind", 10, &GazeboWindPlugin::Callback, this);
...
// This gets called by the world update start event.
    void GazeboWindPlugin::OnUpdate(const common::UpdateInfo &_info) {
        ...
        if (now >= wind_gust_start_ && this->wind_gust_on_) {
    ...
    void GazeboWindPlugin::Callback(const auto_takeoff::WindGust &_wind_gust) {
        this->wind_gust_direction_ = geo_vector3_to_math_vector3_convert(_wind_gust.wind_gust_direction);
        this->wind_gust_force_mean_ = _wind_gust.wind_gust_mean.data;
        this->wind_gust_on_ = _wind_gust.wind_gust_on.data;
    }
